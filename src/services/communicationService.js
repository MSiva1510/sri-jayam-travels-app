...
// ── Enhanced Communication Functions for DAY 37 ────────────────────────
// Queue Management ──────────────────────────────────────────
/**
 * Enqueue a communication for later delivery
 * @param {Object} request - Communication request
 * @returns {Promise<Object>} Queue item with ID
 */
export async function enqueueCommunication(request) {
  // Add to queue manager directly for immediate processing
  const queueId = await queueManager.enqueue({
    channel: request.channels?.[0] || CHANNELS.IN_APP,
    payload: {
      to: `${request.recipient?.type}:${request.recipient?.id}`,
    ...
    logId: null // Will be set by queue manager if needed
  })
  
  // Return the queue item
  const queueItem = await communicationQueueRepository.getById(queueId)
  return queueItem
}
...
export async function getQueueStats() {
  return communicationQueueRepository.getStats()
}
...
export async function getPendingQueue() {
  return communicationQueueRepository.getPending()
}
...
export async function getQueueItem(queueId) {
  return communicationQueueRepository.getById(queueId)
}
...
/**
 * Get providers by channel type
 * @param {string} channel - Channel type (whatsapp, sms, push, webhook, email)
 * @returns {Promise<Array>} Providers for the specified channel
 */
export async function getProvidersByChannel(channel) {
  return providerRepository.getByChannel(channel)
}
...
export async function updateProvider(providerId, updates) {
  return providerRepository.update(providerId, updates)
}
...
/**
 * Get communication logs with enhanced filtering
 ...
 * @returns {Promise<Array>} Statistics by channel
 */
...
/**
 * Check the health of the communication system
 * @returns {Promise<Object>} Health status
 */
export async function checkCommunicationHealth() {
  try {
    ...
    const isHealthy = 
      engineStats.engine.sent >= 0 && // Basic check that engine is working
      queueStats.total >= 0           // Queue is accessible
    
    return {
      healthy: isHealthy,
      timestamp: new Date().toISOString(),
      components: {
        engine: { healthy: true, stats: engineStats },
        queue: { healthy: true, stats: queueStats },
        adapters: engineStats.adapters
      }
    }
  } catch (error) {
    return {
