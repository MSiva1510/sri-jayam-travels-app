...
export async function loadAuditEventsFromDB(limit = 100) {
  if (!supabase) return loadAuditEvents(limit)
  try {
    ...
    return [...pending, ...(data || [])].slice(0, limit)
  } catch {
    // Offline fallback
    return loadAuditEvents(limit)
  }
}
...
export default function AuditLog() {
  return (
    <div className="min-h-screen p-6 bg-slate-50 dark:bg-navy-950 text-slate-900 dark:text-white">
      <div className="mx-auto max-w-3xl rounded-3xl border border-slate-200/80 dark:border-navy-700 bg-white dark:bg-navy-900 shadow-lg p-8">
        <h1 className="text-3xl font-bold mb-3">Audit Log</h1>
        <p className="text-sm text-slate-600 dark:text-slate-400">
          Audit log page is temporarily unavailable. Reload the app or check the audit log admin bundle.
        </p>
      </div>
    </div>
  )
